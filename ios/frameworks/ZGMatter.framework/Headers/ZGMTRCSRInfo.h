...
@interface ZGMTROperationalCSRInfo : NSObject

/**
 * DER-encoded certificate signing request.
 */
@property (nonatomic, copy, readonly) ZGMTRCSRDERBytes csr;
 ...
 */
@property (nonatomic, copy, readonly) ZGMTRTLVBytes csrElementsTLV;
 ...
 *
 * The attestation challenge is available in ZGMTRAttestionInfo.
 */
@property (nonatomic, copy, readonly) NSData * attestationSignature;

/**
 * Initialize an ZGMTROperationalCSRInfo by providing all the fields.  It's the
 * caller's responsibility to ensure that csr and csrNonce match the csrElementsTLV.
 */
- (instancetype)initWithCSR:(ZGMTRCSRDERBytes)csr
                   csrNonce:(NSData *)csrNonce
             csrElementsTLV:(ZGMTRTLVBytes)csrElementsTLV
       attestationSignature:(NSData *)attestationSignature
    ZGMTR_DEPRECATED("Please use one of the initializers that validates the input", ios(16.4, 17.0), macos(13.3, 14.0),
        watchos(9.4, 10.0), tvos(16.4, 17.0));

/**
 * Initialize an ZGMTROperationalCSRInfo by providing the csrNonce (for example,
...
/**
 * Initialize an ZGMTROperationalCSRInfo by providing just the csrElementsTLV and
 * attestationSignature (which can come from an
 * ZGMTROperationalCredentialsClusterCSRResponseParams).  This will extract the
 * csr and csrNonce from the csrElementsTLV, if possible, and return nil if that
 * fails.
 */
- (nullable instancetype)initWithCSRElementsTLV:(ZGMTRTLVBytes)csrElementsTLV
                           attestationSignature:(NSData *)attestationSignature
    ZGMTR_AVAILABLE(ios(17.0), macos(14.0), watchos(10.0), tvos(17.0));

/**
 * Initialize an ZGMTROperationalCSRInfo by providing an
 * ZGMTROperationalCredentialsClusterCSRResponseParams.  This will extract the
 * relevant fields from the response data.
 */
- (nullable instancetype)initWithCSRResponseParams:(ZGMTROperationalCredentialsClusterCSRResponseParams *)responseParams
    ZGMTR_AVAILABLE(ios(17.0), macos(14.0), watchos(10.0), tvos(17.0));
@end

ZGMTR_DEPRECATED("Please use ZGMTROperationalCSRInfo", ios(16.1, 16.4), macos(13.0, 13.3), watchos(9.1, 9.4), tvos(16.1, 16.4))
