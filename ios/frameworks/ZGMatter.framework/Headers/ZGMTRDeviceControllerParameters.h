...
/**
 * Parameters that can be used to initialize an ZGMTRDeviceController.  Specific
...
@interface ZGMTRDeviceControllerAbstractParameters : NSObject
...
/**
 * Parameters that can be used to initialize an ZGMTRDeviceController which
 * has a node identity.
 */
ZGMTR_AVAILABLE(ios(17.6), macos(14.6), watchos(10.6), tvos(17.6))
@interface ZGMTRDeviceControllerParameters : ZGMTRDeviceControllerAbstractParameters
...
/**
 * Set an ZGMTROperationalCertificateIssuer to call (on the provided queue) when
 * operational certificates need to be provided during commissioning.
 */
- (void)setOperationalCertificateIssuer:(id<ZGMTROperationalCertificateIssuer>)operationalCertificateIssuer
                                  queue:(dispatch_queue_t)queue;

/**
 * Set an ZGMTROTAProviderDelegate to call (on the provided queue).  Only needs to
 * be called if this controller should be able to handle OTA for devices.
 */
- (void)setOTAProviderDelegate:(id<ZGMTROTAProviderDelegate>)otaProviderDelegate queue:(dispatch_queue_t)queue;
 ...
 */
@property (nonatomic, assign) NSUInteger concurrentSubscriptionEstablishmentsAllowedOnThread ZGMTR_AVAILABLE(ios(17.6), macos(14.6), watchos(10.6), tvos(17.6));

/**
 * Sets the storage behavior configuration - see ZGMTRDeviceStorageBehaviorConfiguration.h for details
 *
 * If this value is nil, a default storage behavior configuration will be used.
 */
@property (nonatomic, copy, nullable) ZGMTRDeviceStorageBehaviorConfiguration * storageBehaviorConfiguration;

@end

ZGMTR_AVAILABLE(ios(17.6), macos(14.6), watchos(10.6), tvos(17.6))
@interface ZGMTRDeviceControllerExternalCertificateParameters : ZGMTRDeviceControllerParameters
 ...
 *                             ZGMTRDeviceControllerStorageDelegate documentation
 ...
 *                                operationalCertificate.
 */
- (instancetype)initWithStorageDelegate:(id<ZGMTRDeviceControllerStorageDelegate>)storageDelegate
 ...
 */
@property (nonatomic, copy, readonly) ZGMTRCertificateDERBytes rootCertificate ZGMTR_NEWLY_AVAILABLE;

@end

ZGMTR_NEWLY_AVAILABLE
@interface ZGMTRXPCDeviceControllerParameters : ZGMTRDeviceControllerAbstractParameters

- (instancetype)init NS_UNAVAILABLE;
+ (instancetype)new NS_UNAVAILABLE;

/**
 * A controller created from this way will connect to a remote instance of an ZGMTRDeviceController loaded in an XPC Service
 *
...
@property (atomic, readonly, retain) NSUUID * uniqueIdentifier ZGMTR_NEWLY_AVAILABLE;
@property (readonly, strong, nonatomic) NSXPCConnection * (^xpcConnectionBlock)(void) ZGMTR_NEWLY_AVAILABLE;
