...
/**
 * Class that configures how ZGMTRDevice objects persist their attributes to storage, so as to not
 * overwhelm the underlying storage system.
 */
ZGMTR_AVAILABLE(ios(17.6), macos(14.6), watchos(10.6), tvos(17.6))
@interface ZGMTRDeviceStorageBehaviorConfiguration : NSObject <NSCopying>
 ...
 *
 * Each time a report comes in, ZGMTRDevice will wait reportToPersistDelayTime before persisting the
 * changes to storage. If another report comes in during this internal, ZGMTRDevice will wait another
 ...
 *
 * ZGMTRDevice will also track recentReportTimesMaxCount number of report times. If the running
